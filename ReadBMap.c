...
#define READLEN     8384  //1024*8
#define RBUFSIZE    1048576  //1M
...
int  ReadPreKeyarray[1000]={0},RvP[1000]={0},Word[1000]={0},RWord[1000]={0},*pWord[1000]={NULL},*pEnd[1000]={NULL};  //store the PreKey position&HashWord in the short read
int  outFLAG,tempCiagr[10],getLengthout[3],out[2][100]={0},Rout[2][100]={0},exactMatch=0, mappedNum,doubleMap,read_count,Wsize,RWsize;//out[]&Rout[] store the map position info
char outCiagr[100];
int  setNum,ReadLength,repeat2[4], setArray[1000]={0},setOrder[1004]={0}; //maybe the could be a struct datatype
...
int writeOutArray(int repeat, int n, int *Rp,int status)
{
	...
	if( repeat>2)  //3 4 5
	{
		...
		if(setArray[maxOrder]-setArray[minOrder] > CONSIDERLEN)
		{ o[0]=1;	return 1;		}
...
{	
	...
	writeOutArray(RrepeatNum,RWsize,RvP,0); 
...
int match(char *readStr, char *srcBuf, char *RStr, FILE*fp) //count : mapped number!
{
	...
	*(readStr+i)='\0';
	...
	if( result==2)
	{
		i=*out[0]; j=*Rout[0]; temp=ReadLength/2;
		if(out[1][i]+8<temp && Rout[1][j]+8<temp)  //match to start
		{
		...
		else if(out[1][1]>temp && Rout[1][1]>temp)  // match to end
		{
			//-------------reverse  part-----------------
			getLength(Rout[0][1],Rout[1][1],RStr);//out[0] is src posi; out[1] is read offset		
			tempCiagr[0]=-1;
...
int map(char *readFileName, char *buf, char *outFileName, char *argv[])  //main func call
{
	...
	while( fgets(tempStr,READLEN,readFile)!=NULL )
	{
		...
		if(readStr) //there is a short read
		{
			read_count++;		
	...
	printf("exactMatch: %d\n",exactMatch);
	printf("mappedNum: %d;\ndouble Mapped: %d\nrate: %f\n",mappedNum,doubleMap,(float)doubleMap/mappedNum);
